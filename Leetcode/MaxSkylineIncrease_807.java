...
class Solution {
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        ...
        //Store max value of every row and column in rows and cols respectively
        ...
        //Now, calculate max increase possible for the city skyline
        int sum = 0;
        for(int i = 0; i < grid.length; i++) {
            for(int j = 0; j < grid[0].length; j++) {
                int min = rows[i] < cols[j] ? rows[i]:cols[j];
                if(grid[i][j] < min)
                    sum += min-grid[i][j];
            }
        }
